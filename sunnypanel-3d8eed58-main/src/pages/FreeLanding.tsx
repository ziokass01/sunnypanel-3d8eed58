    ...
    fetchFreeConfig()
      ...
      .catch((e: any) => {
        ...
        if (code === "FETCH_FAILED") {
          const backend = (import.meta as any)?.env?.VITE_SUPABASE_URL || "(missing VITE_SUPABASE_URL)";
          setErr(
            `Failed to fetch: Không gọi được backend. (backend: ${backend}) Gợi ý: (1) CORS allow origin cho domain hiện tại, (2) backend URL/project mismatch, (3) backend functions chưa deploy đúng môi trường.`,
          );
          return;
        }
