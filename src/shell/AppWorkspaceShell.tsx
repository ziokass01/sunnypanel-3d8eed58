...
const APP_META: Record<string, { label: string; note: string }> = {
  "find-dumps": {
    label: "Find Dumps",
    note: "Khu điều hành riêng cho app này. Runtime và cấu hình được tách gọn khỏi admin tổng để mobile dễ dùng hơn.",
  },
  "free-fire": {
    label: "Free Fire",
    note: "Khu điều hành riêng của app Free Fire. Giữ nhịp UI gần trang thuê nhưng nghiệp vụ vẫn là server app admin-only.",
  },
};
...
function resolveAppMeta(appCode: string) {
  return APP_META[appCode] ?? {
    ...
    note: "Khu điều hành riêng của app. Logic server giữ nguyên, chỉ làm gọn điều hướng và giao diện để đỡ rối hơn.",
  };
}
        ...
        <div className="overflow-hidden rounded-[28px] border-slate-800 bg-[linear-gradient(135deg,#0b1220_0%,#111827_100%)] text-white shadow-[0_22px_55px_rgba(2,6,23,0.35)]">
          <div className="p-4">
            <div className="flex items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="truncate text-xs text-slate-400">Khu riêng cho runtime và cấu hình</div>
                </div>
              </div>
              <Button
                type="button"
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 text-xs">
              ...
              <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-slate-400">Chỉ giữ 2 cửa chính để đỡ rối</span>
            </div>
          </div>
        </div>
