  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {APPS.map((app) => {
          const workspaceEntry = app.code === "find-dumps"
            ? buildWorkspacePath("find-dumps", "keys", workspaceScope, "", "", location.pathname)
            : app.serverUrl;
          return (
          <Card key={app.code} className="overflow-hidden rounded-[28px] border-slate-200/80 shadow-[0_20px_50px_rgba(15,23,42,0.06)]">
            ...
            <CardContent className="space-y-4">
              <div className="rounded-2xl border bg-muted/20 p-3 text-xs text-muted-foreground break-all">Điểm vào: {workspaceEntry}</div>
              {app.code === "free-fire" ? (
                ...
                <div className="grid gap-2">
                  <Button className="w-full" onClick={() => openTarget(workspaceEntry)}>
