...
const STATUS_OPTIONS = [
  ...
  { value: "disabled", label: "disabled - tắt" },
] as const;
...
export function AdminSunnyModAIPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {tab === "keys" && (
        <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <Card><CardHeader><CardTitle>Tạo key vượt mở token</CardTitle><CardDescription>Prefix mặc định AI-SUNNY, tách khỏi key Free Fire / Find Dumps / Fake Lag.</CardDescription></CardHeader><CardContent className="space-y-3">
            ...
            <Button onClick={() => action.mutate({ action: "create_redeem_key", key: keyDraft })} disabled={action.isPending}>Tạo key vượt</Button>
          </CardContent></Card>
          <Card><CardHeader><CardTitle>Key đã tạo</CardTitle><CardDescription>Key disabled/blocked có nút mở block. Có reset used_count/log để test lại.</CardDescription></CardHeader><CardContent className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Key</TableHead><TableHead>Gói</TableHead><TableHead>Limit</TableHead><TableHead>Used</TableHead><TableHead>Status</TableHead><TableHead /></TableRow></TableHeader><TableBody>{keys.map((k) => <TableRow key={k.id}><TableCell><div className="font-mono text-xs">{k.code_mask}</div><div className="text-xs text-muted-foreground">{k.title}</div></TableCell><TableCell>{k.plan_code_to_grant}<div className="text-xs text-muted-foreground">{k.grant_hours}h</div></TableCell><TableCell>IP {k.daily_ip_limit} / Device {k.daily_device_limit}<div className="text-xs text-muted-foreground">day {k.max_uses_per_day}, total {k.max_uses_total}</div></TableCell><TableCell>{num(k.used_count)}</TableCell><TableCell>{badge(k.status)}</TableCell><TableCell className="min-w-[250px] space-x-2">{k.status === "active" ? <Button size="sm" variant="destructive" onClick={() => action.mutate({ action: "update_redeem_key_status", id: k.id, status: "blocked" })}><Ban className="mr-1 h-3 w-3" />Block</Button> : <Button size="sm" variant="soft" onClick={() => action.mutate({ action: "update_redeem_key_status", id: k.id, status: "active" })}><Unlock className="mr-1 h-3 w-3" />Mở block</Button>}<Button size="sm" variant="outline" onClick={() => action.mutate({ action: "reset_redeem_key_usage", id: k.id })}><RotateCcw className="mr-1 h-3 w-3" />Reset</Button><Button size="sm" variant="destructive" onClick={() => action.mutate({ action: "delete_redeem_key", id: k.id })}><Trash2 className="mr-1 h-3 w-3" />Xóa</Button></TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
        </div>
      )}
