...
struct Options {
    ...
    int poll_interval_ms = 12 * 60 * 1000; // 12m: logout within ~10-15m while saving Supabase verify quota
    int max_auth_lease_seconds = 15 * 60; // local auth lease cap; never trust fake remaining for days
...
SLX9_FORCE_INLINE bool x44(const std::string& response,
    ...
    // Anti fake-server / URL-hook: ok=true must be signed by the real server.
    // The signature binds this response to the request nonce + key + device + build id.
    {
        std::string sigErr;
        std::string response_secret = x45();
        const bool signed_ok = SunnyAntiCrack::VerifySignedOkResponse(
            response,
            request_nonce,
            request_key,
            request_device,
            SunnyAntiCrack::BuildId(),
            [&](const std::string& canonical) -> std::string {
                return HmacSha256Hex(response_secret, canonical);
            },
            &sigErr
        );
        detail::WipeString(response_secret);
        if (!signed_ok) {
            S().lic = LicState{};
            S().remaining_sync_tp = std::chrono::steady_clock::now();
            S().remaining_seconds_at_sync = -1;
            if (err_out) *err_out = sigErr.empty() ? "SERVER_SIG_REQUIRED" : sigErr;
            return false;
        }
    }
...
SLX9_FORCE_INLINE std::string x43(const std::string& key,
                                            ...
                                            std::string* out_request_nonce = nullptr) {
    if (out_err) out_err->clear();

    std::string guard_err;
    if (!SunnyAntiCrack::ValidateConfiguredUrl(S().options.api_url, &guard_err)) {
        if (out_err) *out_err = guard_err;
        return "";
    }
    if (SunnyAntiCrack::DetectKnownUrlHookTool(&guard_err)) {
        if (out_err) *out_err = guard_err;
        return "";
    }
    ...
    std::string body = lit::X114() + JsonEscape(key) +
                       ...
                       lit::X116() + JsonEscape(device_name) +
                       "\",\"build_id\":\"" + JsonEscape(SunnyAntiCrack::BuildId()) + "\"}";

    std::string ts = std::to_string((long long)std::time(nullptr));
    std::string nonce = GenNonceHex(16);
    if (out_request_nonce) *out_request_nonce = nonce;
    ...
    std::string h4 = std::string("x-build-id: ") + SunnyAntiCrack::BuildId();
    ...
    headers = curl_slist_append(headers, h4.c_str());
    ...
    auto do_perform = [&](bool insecure) -> CURLcode {
        response.clear();
        setup_ssl(insecure);
        SunnyAntiCrack::ApplyCurlHardening(curl);
    ...
    curl_easy_getinfo(curl, CURLINFO_EFFECTIVE_URL, &effective_url_c);
    const std::string effective_url = effective_url_c ? std::string(effective_url_c) : S().options.api_url;
    std::string effective_err;
    const bool effective_ok = SunnyAntiCrack::ValidateEffectiveUrl(effective_url, &effective_err);

    if (headers) curl_slist_free_all(headers);
    curl_easy_cleanup(curl);

    if (!effective_ok) {
        if (out_err) *out_err = effective_err.empty() ? "APP_GUARD_EFFECTIVE_URL_BLOCKED" : effective_err;
        return "";
    }
...
SLX9_FORCE_INLINE void x40() {
    ...
    const std::string response = x43(key, device, device_name, &err, &request_nonce);
...
SLX9_FORCE_INLINE void x42() {
    ...
    std::thread([key, dev, dname]() {
        std::string err;
        std::string request_nonce;
        const std::string resp = x43(key, dev, dname, &err, &request_nonce);
        bool ok = false;
