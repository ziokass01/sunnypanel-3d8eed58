...
import { buildGtrafficApiUrl, isGtrafficBlockedResponse, isGtrafficEdgeIpBlock, isQuotaExhaustedError, normalizeShortlinkMode, orderedProvidersForPass, parseGtrafficResponse, providerIsExhaustedToday, providerIsTemporarilyUnavailable, vietnamDate, } from "./free-shared/gtraffic.js";
...
function outboundConcealsGateSecret(outboundUrl, gateUrl) {
    ...
    for (let i = 0; i < 4; i += 1) {
        if ((gateToken && candidate.includes(gateToken)) || candidate.includes(target))
            return false;
        try {
...
function renderTemplate(templateRaw, gateUrl, apiToken) {
    ...
    if (!/[?&](url|u|link|target)=/i.test(rendered) && !template.includes("{url") && !/^https?:\/\//i.test(rendered))
        return "";
    return rendered;
}
...
async function shortenWithProvider(provider, gateUrl, env) {
    ...
    if (isLink4M) {
        ...
        requestUrl = renderTemplate(base.includes("{url") || base.includes("{token")
            ? base
            : `${base}${base.includes("?") ? "&" : "?"}api={token}&url={url_enc}`, gateUrl, token);
        const { data, raw } = await readJsonOrText(requestUrl, "link4m");
        const shortUrl = extractShortUrl(data, raw);
        if (!shortUrl)
            throw new Error(String(data?.message || data?.error || "LINK4M_RESPONSE_INVALID"));
    ...
    if (!token && /\{token\}|apikey=|api=|token=|tokenUser=/i.test(apiUrl || requestUrl))
        throw new Error("SHORTLINK_TOKEN_MISSING");
    ...
    if (/\/st\?/i.test(requestUrl) && !isApiRequest)
        return { outboundUrl: requestUrl };
...
async function loadProviders(db, passNo, channel) {
...
async function chooseProvider(db, cfg, passNo, channel) {
    let providers = [];
    try {
        providers = await loadProviders(db, passNo, channel);
    }
    catch {
        providers = [];
    }
...
async function providerCandidates(db, cfg, passNo, selected, channel) {
    let providers = [];
    try {
        providers = await loadProviders(db, passNo, channel);
    }
...
async function shortenWithFailover(db, cfg, passNo, gateUrl, channel, env) {
    const failures = [];
    let selected = null;
    let candidates = [];
    try {
...
export async function handleFreeStart(req, env) {
    ...
    if (!db)
        return json({ ok: false, code: "FREE_NOT_READY", msg: "FREE_NOT_READY" }, 503);
    ...
    const linkChannel = text(body.link_channel, 16).toLowerCase() === "secondary" ? "secondary" : "primary";
    const ip = getIp(req);
    ...
    if (!keyTypeCode)
        return await deny("MISSING_KEY_TYPE");
    ...
    try {
        ...
        return await deny("KEY_TYPE_LOAD_FAILED", { detail: String(error?.message ?? error) });
    }
    ...
    if (linkChannel === "secondary") {
        ...
        const pass2Providers = requiresDoubleGate ? await loadProviders(db, 2, "secondary").catch(() => []) : pass1Providers;
    ...
    try {
        let pending = db.from("licenses_free_sessions").select("session_id", { count: "exact", head: true })
            ...
            .is("revealed_at", null);
        if (fpHash)
            pending = pending.eq("fingerprint_hash", fpHash);
        else
            pending = pending.eq("ip_hash", ipHash);
    ...
    let shortlinkFailures = [];
    try {
        gateUrl = gateUrlFromToken(gateToken, 1, env);
        const shortened = await shortenWithFailover(db, cfg, 1, gateUrl, linkChannel, env);
