...
async function shortenWithFailover(db: any, cfg: any, passNo: number, gateUrl: string, channel: ShortlinkChannel, env: any) {
...
export async function handleFreeStart(req: Request, env: any) {
  ...
  if (!db) return json({ ok: false, code: "FREE_NOT_READY", msg: "FREE_NOT_READY" }, 503);
  ...
  if (linkChannel === "secondary" && bonusRuntime.active && bonusRuntime.disable_secondary) return await deny("BONUS_SECONDARY_DISABLED", { bonus: publicFreeBonus(bonusRuntime) });
