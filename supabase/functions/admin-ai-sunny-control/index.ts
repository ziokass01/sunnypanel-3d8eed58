...
function normalizeStatus(value: unknown, fallback = "active") {
  ...
  if (["active", "blocked", "disabled"].includes(s)) return s;
  return fallback;
}
...
async function findAuthUser(db: any, args: { user_id?: string | null; email?: string | null }) {
  ...
  // Supabase Admin API has no direct email lookup in older clients, so scan modest pages.
  ...
  try {
    if (action === "dashboard") {
      const [configRes, plansRes, keysRes, accessRes, usageRes, redeemLogsRes, sandboxRes] = await Promise.all([
        ...
        db.from("ai_sunny_redeem_keys").select("*").order("created_at", { ascending: false }).limit(80),
        db.from("ai_sunny_user_access").select("*").order("updated_at", { ascending: false }).limit(80),
        db.from("ai_sunny_usage_logs").select("*").order("created_at", { ascending: false }).limit(120),
        db.from("ai_sunny_redeem_logs").select("*").order("created_at", { ascending: false }).limit(120),
