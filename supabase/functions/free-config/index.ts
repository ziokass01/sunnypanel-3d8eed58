...
async function resolvePerAppQuotaSettings(sb: any, appCodes: string[], fallbackFp: number, fallbackIp: number) {
  ...
  // Fake Lag public quota must follow the Server key page:
  // - Quota thiết bị/ngày ở trang free -> server_app_settings.free_daily_limit_per_fingerprint
  //   with license_access_rules.max_devices_per_key as fallback.
  // - Quota IP/ngày ở trang free -> server_app_settings.free_daily_limit_per_ip
  //   with license_access_rules.max_ips_per_key as fallback.
  // - Lượt verify cho key phát ra is runtime verify cap for the issued key, not the /free daily quota.
  ...
  for (const code of uniqueCodes) {
    ...
    if (code === "fake-lag") {
      ...
      const verifyLimitFallback = positiveLimit(fakeLagRule?.max_verify_per_key);
      const fallbackFpLimit = positiveLimit(fallbackFp);
      const syncedIpLimit = positiveLimit(hit?.free_daily_limit_per_ip);
      const ruleIpLimit = positiveLimit(fakeLagRule?.max_ips_per_key);
      const fallbackIpLimit = positiveLimit(fallbackIp);

      map[code] = {
        free_daily_limit_per_fingerprint: syncedFpLimit ?? ruleFpLimit ?? verifyLimitFallback ?? fallbackFpLimit ?? 0,
        free_daily_limit_per_ip: syncedIpLimit ?? ruleIpLimit ?? fallbackIpLimit ?? 0,
      };
      continue;
    }
    map[code] = {
