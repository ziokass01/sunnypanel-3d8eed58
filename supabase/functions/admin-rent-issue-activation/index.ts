...
Deno.serve(async (req) => {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const code_hash = await sha256Hex(normalizeCode(code));
    const code_last4 = normalizeCode(code).slice(-4);
    ...
    if (!error) {
      return json({ ok: true, code, code_last4, duration_seconds: parsed.data.duration_seconds }, { status: 200, headers: cors });
    }

    // collision -> retry
    if (!error.message.toLowerCase().includes("duplicate")) {
      return json({ ok: false, code: "INSERT_FAILED", msg: error.message }, { status: 500, headers: cors });
    }
  }
