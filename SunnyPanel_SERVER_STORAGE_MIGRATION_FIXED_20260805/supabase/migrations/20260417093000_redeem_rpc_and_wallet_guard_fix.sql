...
declare
  v_row record;
begin
  select uses.id, uses.redeem_key_id
    into v_row
  from public.server_app_redeem_key_uses uses
  where uses.id = p_use_id
  for update;
  ...
  delete from public.server_app_redeem_key_uses where server_app_redeem_key_uses.id = p_use_id;

  update public.server_app_redeem_keys as rk
  set redeemed_count = greatest(0, coalesce(rk.redeemed_count, 0) - 1),
      updated_at = now()
  where rk.id = v_row.redeem_key_id;

  return true;
end;
$$;

create index if not exists idx_server_app_wallet_balances_top_monitor
  on public.server_app_wallet_balances(app_code, soft_balance desc, premium_balance desc, updated_at desc);
