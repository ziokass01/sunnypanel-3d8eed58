...
export type MaybeError = string | ErrorLike | null | undefined;
...
function prettifyField(pathValue: unknown): string {
  ...
  if (Array.isArray(pathValue) && pathValue.length > 0) {
    last = String(pathValue[pathValue.length - 1] ?? "");
...
function translateIssue(issue: any): string | null {
  ...
  if (message) {
    const known = translateKnownText(message, field);
...
function translateParsed(data: unknown): string | null {
  ...
  if (Array.isArray(data)) {
    for (const item of data) {
      const translated = translateIssue(item);
  ...
  if (typeof data === "object") {
    ...
    if (msg) {
      const known = translateKnownText(msg);
      return known || msg;
    }
  }
...
export function getErrorMessage(
  ...
  const directCode =
    typeof error === "object" && error
      ? toText((error as any).code).toLowerCase()
      : "";
  ...
  if (typeof error === "string") {
    ...
    const translatedParsed = translateParsed(parsed);
    if (translatedParsed) return translatedParsed;

    const known = translateKnownText(error);
  ...
  for (const text of candidates) {
    ...
    const translated = translateParsed(parsed);
    if (translated) return translated;
  }
