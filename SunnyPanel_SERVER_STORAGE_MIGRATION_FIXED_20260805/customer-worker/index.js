...
function corsHeaders(origin, env) {
  ...
  const headers = {
    ...
    "Access-Control-Allow-Headers": "Content-Type,Authorization,apikey,Hmac,X-Client-Info,X-Gateway-Project,x-ts,x-nonce,x-sig,x-build-id,x-fp,x-admin-key",
  ...
  async fetch(req, env) {
    ...
    if (route.kind === "health") {
      return json({
        ...
        rent_enabled: String(env.RENT_FEATURE_ENABLED || "0").trim() === "1",
      }, 200, origin, env);
    }
    ...
    const rentFunctions = new Set(["rent-verify-key", "rent-user", "admin-rent", "admin-rent-integrations"]);
    const rentEnabled = String(env.RENT_FEATURE_ENABLED || "0").trim() === "1";
    if (rentFunctions.has(fnName) && !rentEnabled) {
      return json({ ok: false, code: "FEATURE_DISABLED", msg: "Rent feature is disabled" }, 410, origin, env);
    }
