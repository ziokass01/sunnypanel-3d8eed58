...
- `POST /api/rent-verify-key` — **đang đóng**, trả `410 FEATURE_DISABLED`
- `POST /api/free-start`
- `POST /api/reset-key`
- `POST /api/admin-rent` — **đang đóng**, trả `410 FEATURE_DISABLED`
...
Rent mặc định đóng. Worker chặn các route Rent trừ khi `RENT_FEATURE_ENABLED=1`. Edge Functions Rent cũng có cùng cổng này.
